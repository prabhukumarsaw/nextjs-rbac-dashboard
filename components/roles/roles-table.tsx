...
/**
 * Roles Table Component - defensive against missing fields
 */
interface Role {
  id: string;
  name?: string | null;
...
export function RolesTable({ roles }: RolesTableProps) {
  ...
  // Defensive normalization: ensure roles is an array
        ...
        <Table>
          ...
          <TableBody>
            {safeRoles.length === 0 ? (
              ...
              safeRoles.map((role) => {
                // Normalize permissions and menus to arrays to avoid calling .slice on undefined
                const permissions = Array.isArray(role?.permissions) ? role.permissions : [];
                const menus = Array.isArray(role?.menus) ? role.menus : [];

                const name = role?.name ?? "(no name)";
                const slug = role?.slug ?? "(no slug)";
                const userCount = typeof role?.userCount === "number" ? role.userCount : 0;
                const isActive = Boolean(role?.isActive);

                return (
                  <TableRow key={role.id}>
                    <TableCell className="font-medium">{name}</TableCell>
                    <TableCell>
                      <code className="text-xs bg-muted px-2 py-1 rounded">{slug}</code>
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-1 flex-wrap max-w-md">
